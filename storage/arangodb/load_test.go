...
func TestLoadOboJSONFromDataSource(t *testing.T) {
	...
	assert.Equal(1143, info.RelationStats, "should load 1143 relationships")
	assert.Equal(1271, info.TermStats.Created, "should load 1271 terms")
	assert.Equal(0, info.TermStats.Updated, "should have not updated any term")
	assert.Equal(0, info.TermStats.Deleted, "should have not deleted any term")
	r2 := oboReader(assert)
	...
	assert.Equal(0, info2.RelationStats, "should not load any relationships")
	assert.Equal(0, info2.TermStats.Created, "should not load any terms")
	assert.Equal(1271, info2.TermStats.Updated, "should have updated 1271 term")
	assert.Equal(0, info2.TermStats.Deleted, "should have not deleted any term")
}
